...
// DwdDayRootGameReg30BackUidLoginLogModel 根注册30天回流按天日志表
type DwdDayRootGameReg30BackUidLoginLogModel struct {
	sql.SqlBaseModel
	sql.SqlCommonModel
...
func (receiver *DwdDayRootGameReg30BackUidLoginLogModel) TableName() string {
	return "dwd_day_root_game_reg_30_back_uid_login_log"
}

func (receiver *DwdDayRootGameReg30BackUidLoginLogModel) Take(ctx context.Context, fields string, query string, args ...interface{}) (err error) {
	err = receiver.Db().WithContext(ctx).Table(receiver.TableName()).Select(fields).Where(query, args...).Take(receiver).Error
	return
}

func (receiver *DwdDayRootGameReg30BackUidLoginLogModel) Create(ctx context.Context) (err error) {
