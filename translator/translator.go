...
func removeTopStruct(fields map[string]string) map[string]interface{} {
	...
	for field, err := range fields {
		...
		lowerMap[fieldArr[len(fieldArr)-1]] = err
	}
...
// handler中调用的错误翻译方法
func DealErr(err error) (resp error) {
	errs, ok := err.(validator.ValidationErrors)
	if !ok {
		resp = err
		return
	}
	errsTmp := removeTopStruct(errs.Translate(trans))
	for _, value := range errsTmp {
		return errors.New(cast.ToString(value))
	}
	return
}
